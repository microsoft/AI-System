...
[主导资源公平简称DRF（Dominant Resource Fairness）](https://cs.stanford.edu/~matei/papers/2011/nsdi_drf.pdf)调度使用优势资源的概念来比较多维（CPU，GPU，内存等）资源。这个想法是在多资源环境中，资源分配应该由作业（用户或队列）的主导份额决定，这是作业已分配的任何资源（内存或 CPU）的最大份额。其论文中介绍，与其他可能的策略不同，DRF 满足几个较为理想的属性。首先，DRF 激励用户共享资源，进而保证公平性。其次，DRF 是[策略证实（Strategy-Proof）](https://en.wikipedia.org/wiki/Strategyproofness)的，因为用户不能通过谎报需求来增加作业资源的分配。第三，DRF 是[无嫉妒（Envy-free）](https://en.wikipedia.org/wiki/Envy-freeness)的，因为没有用户愿意将配额与另一个用户的配额进行交易。最后，DRF 分配是帕累托有效的，因为不可能在不减少另一个用户的分配的情况下改善一个用户的分配。作者在 Mesos 集群资源管理器中实现了 DRF。
