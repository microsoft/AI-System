...
在已有的工作中（例如，[Gandiva OSDI '18](https://dl.acm.org/doi/10.5555/3291168.3291212) [<sup>[2]</sup>](#gandiva)）对当前深度学习作业负载的特点进行了分析与总结。我们使用的集群管理系统管理的大多数作业是深度学习的训练作业。
深度学习作业相比传统的批处理作业（例如，大数据分析作业）有一些新的特点和不同。
“
- 单个深度学习训练作业特点：
  - 训练时间持续数小时甚至几天。
