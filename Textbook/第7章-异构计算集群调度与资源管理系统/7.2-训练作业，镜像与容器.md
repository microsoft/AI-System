...
Docker镜像层原理： 
- 含有可读和读写层: 其包含多种层。
  - 基层(Base Layer)：这是文件系统的基本文件的存储位置，该层（来自叠加视图）是只读的。如果从Docker镜像的角度考虑这一点，可以将此层视为您的基础镜像。 
  - 叠加层(Overlay Layer): 叠加层是用户操作的地方，它最初提供基础层的视图，并让用户能够与文件交互甚至"写入"文件。当用户写入此层时，更改将存储在下一层中。当进行更改时，该层将提供基层和差异层的联合视图，其中差异层的文件取代了基层的文件。同样，如果您想从Docker镜像的角度考虑这一点，您可以将此层视为运行容器时所看到的层。
  - 差异层(Diff Layer)：在叠加层中所做的任何更改都会自动存储在该层中。
