...
MLOps 是一种用于机器学习和深度学习工程的方法，它将机器学习（ML部分）开发与机器学习系统统一起来操作与运维（Ops部分）。MLOps其倡标准化和（在有益的情况下）自动化机器学习系统的关键步骤。 MLOps 提供了一套标准化的流程和技术能力，用于构建、部署、快速可靠地运行机器学习系统。MLOps 以DevOps和DataOps支持应用程序工程和数据工程（分析）的方式支持机器学习开发和部署。相比于传统的DevOps不同之处在于，当用户部署Web服务时，用户关心的是弹性(resilience)、每秒查询数(QPS)、负载平衡(Load Balance)等。在部署机器学习模型时，用户还需要关注数据的变化、模型的变化等等。 这就是 MLOps所要解决的问题。 
