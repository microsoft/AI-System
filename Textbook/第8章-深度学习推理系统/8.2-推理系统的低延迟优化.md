...
推理系统类似传统的Web服务，需要响应用户请求，并保证一定的服务等级协议，进而需要通过一定的策略和优化，保持一定的低延迟，本小节将围绕部署过程中涉及到的延迟问题和优化进行展开。
...
我们将模型抽象为计算图，其中在设备执行的层(layer)在一些框架内也称作内核(kernel)。我们可以从下图中看到，设备中执行一个内核，一般需要以下几个步骤和时间开销，启动内核，读取数据到设备，执行内核，结果数据写回主存。
...
- 约束：保证延迟SLA
- 策略：
  - 缓存分配器(Cached Allocator)：推理系统预先申请设备内存，构建内存管理器，减少设备内存分配释放等API调用代价。
  - 预取(Pre-fetching)和卸载(Off-loading)：异步地将设备内存数据在读取前和产生后和主存进行换入换出，减少设备内存的数据压力。
  - 算子融合(Fusion)：将中间结果在缓存层给下一阶段的内核使用，减少中间结果回写设备内存。

## 8.2.5 低精度推理与精度校准

推理阶段相比训练阶段可以适当降低精度，进而降低推理延迟:
- 对模型进行充分训练后，由于不需要进行反向传播求梯度，因此推理计算可以使用半精度FP16甚至INT8张量运算
...
本小节主要围绕推理系统的延迟展开讨论，我们总结了低延迟的推理需求，以及围绕这个设计目标，推理系统常常使用的优化策略。优化延迟的目标，受到空间与准确度的约束，在适当情况下可以牺牲一定准确度进而提升延迟。
