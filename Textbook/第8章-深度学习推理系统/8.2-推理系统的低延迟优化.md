...
- [8.2 推理系统的低延迟优化](#82-推理系统的低延迟优化)
  ...
  - [8.2.4 内存分配策略调优](#824-内存分配策略调优)
    - [深度学习模型内存分配算法实验与模拟研究](#深度学习模型内存分配算法实验与模拟研究)
    - [(1) 数据获取](#1-数据获取)
    - [(2) 评测指标设定](#2-评测指标设定)
    - [(3) 算法实现与评测](#3-算法实现与评测)
...
2. 程序基准内存消耗预估：假设A,B,Result的元素数据类型为Float32 (32 bit)，预估程序整体的内存占用(Byte为单位)。
3. 思考如果元素数据类型换为Int8 (8 bit)，内存占用会为多少？MAC会降低为多少？
   - 本问题启示大家思考模型量化的作用和价值
...
def func1(A, B, result1):
    ...
    return result2
```
6. 请思考，如果执行此程序的设备缓存线([Cache line](https://stackoverflow.com/questions/3928995/how-do-cache-lines-work))为3，有2个缓存线，预估缓存失效([Cache misses](https://en.wikipedia.org/wiki/Cache_performance_measurement_and_metric#Introduction_to_types_of_cache_misses))率?思考设计缓存失效率最低的访问方式?如果有两个core可以同时启动两个线程计算，之后会如何设计并行计算的策略？
...
```shell
# 日志收集
# 参考 https://stackoverflow.com/questions/36331419/tensorflow-how-to-measure-how-much-gpu-memory-each-tensor-takes/43189934#43189934 
export TF_CPP_MIN_VLOG_LEVEL=2 # 假设TensorFlow 1.13版本
...
读者可以选用以经典算法作为基准测试（例如，[DNNMem](https://dl.acm.org/doi/10.1145/3368089.3417050)中介绍的主流框架内存分配算法，或者传统[内存分配器](https://www.cs.tufts.edu/~nr/cs257/archive/doug-lea/malloc.html)策略），设计新的算法，并通过收集的数据模拟，看能否提升当前目标，超越基准算法，并进行结果分析，形成分析报告或论文。
