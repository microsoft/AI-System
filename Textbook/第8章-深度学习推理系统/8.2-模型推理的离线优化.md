...
推理系统类似传统的 Web 服务，需要响应用户请求，并保证一定的服务等级协议（例如，响应时间低于 100ms），进而需要通过一定的策略和优化，保持一定的低延迟（Low Latency），本小节将围绕部署过程中涉及到的延迟问题和相应的优化进行展开。 
...
接下下我们以一个[实例](https://gist.github.com/aleju/eb75fa01a1d5d5a785cf)说明如何实现一个简单的量化函数并应用在张量中。此实例对张量每个元素应用用户自定义函数，在函数中，将每个输入元素值转换为 to_values 数组中最接近的值，to_valuues 向量可以由用户设计的算法计算出来，根据最大最小取值约束和数据的数值分布获得。以下实例只是说明一种原理，量化算法多种多样，读者可以参考相关论文实现与理解。
...
读者思考，torch.float32 到 torch.int8 转换后，对 Tensor(4, 5) 计算量和内存优化了多少倍？ 
...
- [Measuring and Optimizing Tail Latency](https://www.cs.utexas.edu/users/mckinley/talks/ds-strangeloop-2017.pdf)
...
- [Jonathan Ragan-Kelley, Connelly Barnes, Andrew Adams, Sylvain Paris, Frédo Durand, and Saman Amarasinghe. 2013. Halide: a language and compiler for optimizing parallelism, locality, and recomputation in image processing pipelines. SIGPLAN Not. 48, 6 (June 2013), 519–530. https://doi.org/10.1145/2499370.2462176](https://dl.acm.org/doi/10.1145/2491956.2462176)
...
- https://nni.readthedocs.io/en/v1.6/Compressor/Overview.html
