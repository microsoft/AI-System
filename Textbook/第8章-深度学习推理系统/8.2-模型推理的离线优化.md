...
<center>图 8.2.4 对比不同后端自动调优搜索策略 <a href="https://arxiv.org/pdf/2006.06762.pdf">图片引用自 Ansor</a></center>
...
接下下我们以一个[实例](https://gist.github.com/aleju/eb75fa01a1d5d5a785cf)[<sup>[7]</sup>](#quantize)说明如何实现一个简单的量化函数并应用在张量中。此实例对张量每个元素应用用户自定义函数，在函数中，将每个输入元素值转换为 to_values 数组中最接近的值，to_valuues 向量可以由用户设计的算法计算出来，根据最大最小取值约束和数据的数值分布获得。以下实例只是说明一种原理，量化算法多种多样，读者可以参考相关论文实现与理解。
