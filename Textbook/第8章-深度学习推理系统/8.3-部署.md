...
例如，我们可以通过下面的[实例练习](https://pytorch.org/serve/grpc_api.html)，将训练好的模型部署并通过 gRPC 的 API 调用。这样意味着，我们可以将模型本身变为微服务，融合进当前已有的基础架构中其他的微服务中只需要客户端提供好相应的输入数据，处理好返回数据即可，模型本身当成一个软件服务被使用。
