...
<center>图8-4-2. 加速器的低效率使用</center>
...
[装箱（Bin Packing）](https://en.wikipedia.org/wiki/Bin_packing_problem)：装箱问题是一个组合优化问题，其中不同大小的物品必须装入有限数量的箱或容器中，每个箱或容器具有固定的给定容量，其方式为 最小化使用的箱数量。 该问题有许多应用，例如填充容器、物流车辆装载、大规模平台资源调度等。当前我们可以认为GPU是箱子，而模型就是要装入的物品，将运行时的模型调度抽象为装箱问题。
