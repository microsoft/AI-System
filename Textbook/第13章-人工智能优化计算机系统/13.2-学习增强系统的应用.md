...
ABR 算法的难点之一在于用户的网络状况可能不稳定，带宽会随时间变化。比如，用户在移动网络下观看视频时带宽波动可能较大，这使得 ABR 算法难以简单地根据过去观测到的下载速度来预估未来的网络带宽，经常导致算法误选过高或过低的视频码率，带来卡顿或画质不清晰的问题。ABR 算法的难点之二是每一步的码率决策不只影响当下的视频切片，而是对未来的切片选择也有影响。举例来说，如果 ABR 算法选择请求了高码率的切片，但用户的网络同时突然变差，那么客户端本地的视频缓存则会逐渐耗尽，继而导致 ABR 算法只好选择一些低码率的视频切片来避免潜在的卡顿。也就是说，优化长期平均的 QoE 需要 ABR 算法对未来提前做出规划、进行序列决策（Sequential Decision Making），同时这也表明强化学习（Reinforcement Learning，RL）可以适用于该场景。

为了解决以上挑战、弥补传统 ABR 算法的缺陷，麻省理工学院和斯坦福大学先后提出了基于强化学习的 ABR 算法。这里，我们着重介绍来自斯坦福大学的工作 —— Puffer。首先，为了在真实网络环境中研究视频流，Puffer 搭建了一个大规模流媒体直播平台 [puffer.stanford.edu](https://puffer.stanford.edu)。该平台允许美国的用户免费在线观看六个电视频道，并部署了多种 ABR 算法进行随机对照实验。迄今为止，Puffer 已有超过 20 万的真实用户，收集了上百年长度的视频观看数据，并在每天自动发布匿名后的数据供公众进行分析研究。与此同时，Puffer 论文也提出了一种新的基于机器学习的 ABR 算法 —— Fugu。在不影响理解的前提下，我们简化并描述 Fugu 算法如下。

为了解决 ABR 算法的难点一，Fugu 训练了一个名为 TTP（Transmission Time Predictor）的神经网络来预测给定视频切片的传输时间，这也是网络为自适应流系统带来的主要不确定变量。TTP 的输入包含（1）过去下载的 *t* 个视频切片的大小；（2）过去下载的 *t* 个视频切片的传输时间；（3）Puffer 服务器上的 TCP 统计信息；以及（4）正在考虑下载的某码率视频切片的大小。TTP 的输出正是对这一视频切片选项传输时间的预测。由于在数据收集时可以观测到下载的每一个视频切片的传输时间，那么在训练 TTP 时自然地可以使用标准的监督学习（Supervised Learning）来最小化 TTP 的预测时间与真实传输时间的差距。该训练每隔一段时间在线下进行，并将训练好的模型部署到线上。

针对 ABR 算法的难点之二，Fugu 在线上运行时使用了模型控制预测（Model Predictive Control，MPC）算法来对未来的切片选择做出规划。具体来说，MPC 会向前考虑长度为 *h* 个切片的时间窗口，并枚举该窗口内所有的决策“路径” —— 即由 *h* 个切片的码率依次组成的序列（在实际运行中 Fugu 通过优化避免了枚举）。MPC 对于每条决策路径进行模拟并计算出在该时间窗口内可获得的总 QoE，然后可以选择 QoE 总和最大的路径。虽然如此，在 MPC 执行完最优路径上的第一个决策后，即下载完指定码率的下一个切片时，MPC 会重新使用相同的方法进行规划，从而纳入最新观测到的系统状态（如上一切片的下载时间、最新的视频缓存长度等），用来避免更多的决策误差。

总的来说，Fugu 属于基于模型的强化学习（Model-based RL）。如下图所示，Fugu 在线上运行的 MPC 算法不断从 Puffer 服务器上获取更新的系统状态，并调用 TTP 神经网络来进行自适应的码率选择。每隔一段时间后，Fugu 会整合服务器上最近收集到的数据，于线下使用监督学习来训练 TTP 神经网络，并部署更新后的 TTP 在服务器上。相较于无模型的强化学习（Model-free RL），Fugu 使用的基于模型的强化学习可以依赖更少的样本数据进行更高效且稳定的训练。同时，由于 Fugu 对网络进行了建模，所以其在运行时增加了透明度和可解释性（对比无模型强化学习的“黑盒子”）。

<center><img src="./img/2/fugu.png" width="400"/></center>
<center>图13.2.1. Fugu 使用基于模型的强化学习算法。</center>
<br>

通过长达八个月在 Puffer 真实用户上运行的实验，作者发现 Fugu 的性能优于现有的其它四个算法。由于 QoE 不存在统一定义，所以作者分别比较了组成 QoE 的不同维度。结果显示，在所有的 ABR 算法中，Fugu 实现了最高的视频画质（由一种衡量画质的 SSIM 参数描述），最小的画质浮动（相邻视频切片的平均 SSIM 差），以及（几乎）最低的视频卡顿。数据还显示，Puffer 的用户在 Fugu 算法被使用时观看的时间也比其它算法被使用时更长。
