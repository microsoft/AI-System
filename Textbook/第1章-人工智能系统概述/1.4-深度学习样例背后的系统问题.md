...
<center>图 1-5-3. Conv2D 计算过程实例</center>
...
（4）调度并运行代码：如图 1-4-9 所示，这个阶段框架会将算子及其运行期的代码实现，依次根据依赖关系，调度到计算设备上进行执行。对一些不方便静态做优化的选择，可以通过运行期调度达到，例如，并发（Concurrent）计算与 I/O，如有空闲资源并行执行没有依赖的算子等。
...
Remzi Arpaci-Dusseau（University of Wisconsin—Madison）曾在FAST '19（[Measure, Then Build](https://www.usenix.org/conference/atc19/presentation/keynote#:~:text=This%20talk%20will%20discuss%20an,implementing%20new%20systems%20to%20solve)）和'22（[FAST '22 - 25 Years of Storage Research and Education: A Retrospective](https://www.youtube.com/watch?v=u9RECEzxk6I)）中的演讲中提到系统研与学习工作中度量之后构建[“Measure, Then Build”](https://www.usenix.org/conference/atc19/presentation/keynote#:~:text=This%20talk%20will%20discuss%20an,implementing%20new%20systems%20to%20solve)的方法。
...
其提出：研究是一种学习实践“Research is a learning expercise”，不断思考什么是可度量的，以及如何从中可以学到什么。
