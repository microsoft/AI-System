...
- [1.5 影响深度学习系统设计的理论，原则与假设](#15-影响深度学习系统设计的理论原则与假设)
  ...
  - [1.5.2 摩尔定律(Moore's Law)-预测算力发展趋势](#152-摩尔定律moores-law-预测算力发展趋势)
  - [1.5.3 局部性原则(Priciple of Locality)-内存层次结构(Memory Hierarchy)](#153-局部性原则priciple-of-locality-内存层次结构memory-hierarchy)
...
例如，我们通过一个向量加法的实例，从顶到下，看各个层次是如何抽象和层次化标识具体的执行逻辑的：
- 语言：Python语言
...
add.s64 %rd6, %rd4, %rd5; // 数组a全局地址%rd4加id * 8结果%rd5，得到数组第i元素地址放置于rd6
cvta.to.global.u64 %rd7, %rd2; // 转换%rd2也就是b为全局内存地址%rd7，
add.s64 %rd8, %rd7, %rd5; // 数组b全局地址%rd7加id * 8结果%rd5，得到数组第i元素地址放置于rd8
ld.global.f64 %fd1, [%rd8]; // 加载全局内存数组b的地址[%rd8]（也就是第i个元素）数据到寄存器fd1
ld.global.f64 %fd2, [%rd6]; // 加载全局内存数组a的地址[%rd8]（也就是第i个元素）数据到寄存器fd2
add.f64 %fd3, %fd2, %fd1; // 相加寄存器中a,b第i元素到寄存器%fd3，对应代码a[id] + b[id]
cvta.to.global.u64 %rd9, %rd3; // 转换%rd3也就是c的地址转换为为全局内存地址到寄存器%rd9
add.s64 %rd10, %rd9, %rd5; // 数组c全局地址%rd9加id * 8结果%rd5，得到数组第i元素地址放置于rd10
...
## 1.5.2 摩尔定律(Moore's Law)-预测算力发展趋势
...
## 1.5.3 局部性原则(Priciple of Locality)-内存层次结构(Memory Hierarchy)
