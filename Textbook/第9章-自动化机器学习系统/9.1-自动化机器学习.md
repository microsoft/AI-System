...
**基于预测器的搜索**是指在搜索空间中采样的每个网络结构独立做表现评估。另一种是通过代理获取网络结构的表现，如使用表现预测器(performance predictor)。使用后一种评估方式的多试验搜索算法在研究上更多的侧重于如何获取更加准确的表现预测器。表现预测器通常基于一批网络结构的真实表现拟合得来。如--。这类方法的优势是使用的计算资源相对会少很多，但是其表现预测器的精度也依赖于已知真实表现的网络结构的数量，数量越多精度也会越高。
